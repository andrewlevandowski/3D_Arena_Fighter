...
public class EnemyController : MonoBehaviour {
    ...
    void Start () {
        target = GameSingleton.instance.player.transform;
	...
	void Update () {
        ...
        //follows if within range
        if (distance <= lookRadius)
        {
            agent.SetDestination(target.position);

            //attacks if close enough, goes back to "walking" if not in close range
            if (distance <= 2.8f)
            {
                FaceTarget();
                Attack();
    ...
    //turns robot smoothly towards person
    void FaceTarget()
    {
